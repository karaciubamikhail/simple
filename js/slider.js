...
class ItcSlider {
   ...
   * @param {String} prefix
   */
  constructor(el, config = {}, prefix = 'brand_slider-') {
    this.#state = {
   ...
   * @param {String} prefix
   */
  static getOrCreateInstance(target, config = {}, prefix = 'brand_slider-') {
  ...
  static createInstances() {
    document.querySelectorAll('[data-slider="brand_slider"]').forEach((el) => {
      const { dataset } = el;
      const params = {};
  ...
  #onClick(e) {
    ...
    if (!(e.target.closest('.brand_slider-btn') || e.target.closest('.brand_slider-indicators'))) {
      return;
    }
